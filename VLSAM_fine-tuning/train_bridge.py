...
def main() -> None:
    ...
    def tversky_loss(
        ...
        Default alpha=0.3, beta=0.7 penalises FP more → reduces over-segmentation.
        """
        probs   = torch.sigmoid(logits)
        probs   = probs.reshape(probs.shape[0], -1)
        targets = targets.reshape(targets.shape[0], -1)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_loss < best_val_loss:
            ...
            best_path = ckpt_dir / "bridge_best.pt"
            torch.save(
                {
                    "epoch": epoch,
    ...
    print(f"Best checkpoint saved at: {ckpt_dir / 'bridge_best.pt'}")
