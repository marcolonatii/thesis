...
class SAM2CameraPredictorVOS(SAM2CameraPredictor):
    ...
    @torch.inference_mode()
    def remove_object(self, obj_id, strict=False):
        """
        Remove the specified object (given by obj_id) from the tracking state.
        This function updates all related entries in self.condition_state including:
          - obj_id_to_idx, obj_idx_to_id, and obj_ids
          - point_inputs_per_obj, mask_inputs_per_obj,
            output_dict_per_obj, temp_output_dict_per_obj, and frames_already_tracked
        If strict is True and the obj_id does not exist, an error is raised.
        If there is only one object remaining, the state is reset.
        """
        cs = self.condition_state

        # Check if the object exists.
        old_obj_idx = cs["obj_id_to_idx"].get(obj_id, None)
        if old_obj_idx is None:
            if strict:
                raise RuntimeError(
                    f"Cannot remove object id {obj_id} as it doesn't exist. "
                    f"Existing object ids: {cs['obj_ids']}."
                )
            else:
                return

        # If this is the only object, reset the state.
        if len(cs["obj_id_to_idx"]) == 1:
            self.reset_state()
            return

        # Get current list of object ids and indices.
        old_obj_ids = cs["obj_ids"]  # e.g., [id0, id1, id2, ...]
        old_indices = list(range(len(old_obj_ids)))  # e.g., [0, 1, 2, ...]
        # Remove the index of the object to delete.
        remaining_indices = [i for i in old_indices if i != old_obj_idx]
        # Build new object id list in the order of remaining objects.
        new_obj_ids = [old_obj_ids[i] for i in remaining_indices]
        new_indices = list(range(len(new_obj_ids)))  # new indices: 0, 1, 2, ...

        # Build mapping from old index to new index.
        old_to_new = {old: new for old, new in zip(remaining_indices, new_indices)}

        # Update object id mappings.
        cs["obj_id_to_idx"] = OrderedDict({obj_id: idx for idx, obj_id in enumerate(new_obj_ids)})
        cs["obj_idx_to_id"] = OrderedDict({idx: obj_id for idx, obj_id in enumerate(new_obj_ids)})
        cs["obj_ids"] = new_obj_ids

        # Helper function: update a per-object dictionary by shifting keys using old_to_new mapping.
        def update_per_obj_dict(per_obj_dict):
            new_dict = {}
            for old_idx, value in per_obj_dict.items():
                if old_idx in old_to_new:
                    new_dict[old_to_new[old_idx]] = value
            return new_dict

        cs["point_inputs_per_obj"] = update_per_obj_dict(cs["point_inputs_per_obj"])
        cs["mask_inputs_per_obj"] = update_per_obj_dict(cs["mask_inputs_per_obj"])
        cs["output_dict_per_obj"] = update_per_obj_dict(cs["output_dict_per_obj"])
        cs["temp_output_dict_per_obj"] = update_per_obj_dict(cs["temp_output_dict_per_obj"])
        cs["frames_already_tracked"] = update_per_obj_dict(cs["frames_already_tracked"])
