...
Then, install SAM 2 from the root of this repository:
...
SAM2_BUILD_CUDA=0 uv sync --extra notebooks
```
...
This usually happens because you have multiple versions of dependencies (PyTorch or CUDA) in your environment. During installation, the SAM 2 library is compiled against one version library while at run time it links against another version. This might be due to that you have different versions of PyTorch or CUDA installed separately via `pip` or `uv`. You may delete one of the duplicates to only keep a single PyTorch and CUDA version.
