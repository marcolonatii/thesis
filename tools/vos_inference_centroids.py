...
# hardcoded preview directory (no CLI arg)
PREVIEW_DIR = "/Experiments/marcol01/centroid_preview"
...
def vos_inference(
    ...
    for input_frame_idx in input_frame_inds:
        ...
        for object_id, object_mask in per_obj_input_mask.items():
            ...
            # convert the input mask to a single positive point at the mask centroid
            mask_arr = (
                object_mask
                if isinstance(object_mask, np.ndarray)
                else np.asarray(object_mask)
            )
            ...
            # pass pixel coordinates (x, y); let predictor normalize them
            points = torch.tensor([[centroid_x, centroid_y]], dtype=torch.float32)
            ...
            #out_name = f"{video_name}_frame{frame_names[input_frame_idx]}_obj{object_id}.png"
...
def vos_separate_inference_per_object(
    ...
    for object_id in object_ids:
        ...
        for input_frame_idx in input_frame_inds:
            object_mask = inputs_per_object[object_id][input_frame_idx]
            # convert mask to centroid point and add as positive point
            mask_arr = object_mask if isinstance(object_mask, np.ndarray) else np.asarray(object_mask)
            if not np.any(mask_arr):
                continue
            ys, xs = np.nonzero(mask_arr)
            centroid_x = float(xs.mean())
...
def main():
    ...
    # ensure hardcoded preview dir exists
    os.makedirs(PREVIEW_DIR, exist_ok=True)
